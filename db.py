...
class BotDB:
    ...
    def add_user(self,user_id,date):
        if (self.check_exist_user(user_id) == False):
            self.cursor.execute("INSERT INTO users (user_id, join_date) VALUES (%s, %s)",(user_id,date))
        self.conn.commit()
    ...
    def add_record(self, user_id, place):
        if (self.user_exists(user_id) == False):
            self.cursor.execute("INSERT INTO mailings (user_id, place) VALUES (%s, %s)",(user_id,place))
        else:
